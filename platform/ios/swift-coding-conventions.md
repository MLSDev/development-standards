...
Remove autogenerated code and unused methods and variables. Also try to avoid writing comments - code should be self-explanatory. Comments are only acceptable when they describe not obvious workaround in code, that can not be avoided or made more clear.
...
Minimize parameter names that go inside closures to minimize closure syntax, for example:
